  ...
  try {
    ...
    for (let i = 2; i < data.length; i++) {
      if (data[i][0] && data[i][0].toString() === fundCode.toString()) {
        // Found - parse JSON from columns C and D
        try {
...
function processAssetAllocation(allocation) {
  try {
    ...
    // Create separate JSON objects for asset allocation and equity allocation
    ...
    if (foundRow > 0) {
      ...
      applyDataRowFormatting(sheet, newRow, newRow, 4);
