...
function getMemberDashboardData(memberName) {
  try {
    ...
    const allMF = getAllMutualFundPortfolios() || [];
    const allStocks = getAllStockPortfolios() || [];
    const allOtherInv = getAllOtherInvestments() || [];
...
/**
 * DEBUG: Check email configuration status
 * Run this from Apps Script editor to see current status
 */
function debugEmailConfiguration() {
  ...
  const data = settingsSheet.getDataRange().getValues();
  Logger.log('Settings sheet has ' + data.length + ' rows');
  ...
  // Check for EmailConfigured setting
  let found = false;
  for (let i = 2; i < data.length; i++) {
    if (data[i][0] === 'EmailConfigured') {
      found = true;
      Logger.log('✅ EmailConfigured found at row ' + (i + 1) + ': ' + data[i][1]);
      SpreadsheetApp.getUi().alert(
        '✅ EmailConfigured Status',
        'Current value: ' + data[i][1] + '\n\n' +
        'Row: ' + (i + 1) + '\n' +
        'isEmailConfigured() returns: ' + isEmailConfigured(),
  ...
  if (!found) {
    Logger.log('❌ EmailConfigured setting NOT FOUND in Settings sheet');
    SpreadsheetApp.getUi().alert(
      '❌ EmailConfigured Not Found',
      'The EmailConfigured setting is missing from Settings sheet.\n\n' +
      'Click OK to add it now.',
      SpreadsheetApp.getUi().ButtonSet.OK
    );

    // Add it
    settingsSheet.appendRow(['EmailConfigured', 'FALSE', 'Whether user has configured email settings (TRUE/FALSE)']);
    Logger.log('✅ Added EmailConfigured setting');
    SpreadsheetApp.getUi().alert('✅ EmailConfigured setting added!');
  }
}

/**
 * DEBUG: Manually trigger email configuration check
 * Run this to test the first-time setup (auto-configure to Daily 9 AM)
 */
function testEmailConfigurationPrompt() {
  Logger.log('Starting manual email configuration check...');

  const configured = isEmailConfigured();
  Logger.log('isEmailConfigured() = ' + configured);

  if (!configured) {
    Logger.log('Email not configured - auto-configuring to Daily 9 AM');
    showFirstTimeEmailSetup();
  } else {
    Logger.log('Email already configured');
    SpreadsheetApp.getUi().alert(
      'ℹ️ Already Configured',
      'Email settings are already configured.\n\n' +
      'To test the first-time setup:\n' +
      '1. Open Settings sheet\n' +
      '2. Change EmailConfigured from TRUE to FALSE\n' +
...
/**
 * DEBUG: Reset email configuration (for testing)
 * Sets EmailConfigured to FALSE so you can test the first-time dialog
 */
  ...
  try {
    ...
    for (let i = 2; i < data.length; i++) {
      if (data[i][0] === 'EmailConfigured') {
        ...
        Logger.log('✅ Reset EmailConfigured to FALSE');
        SpreadsheetApp.getUi().alert(
          '✅ Reset Complete',
          'EmailConfigured has been set to FALSE.\n\n' +
          'Close and reopen the spreadsheet to see auto-configuration to Daily 9 AM.',
          SpreadsheetApp.getUi().ButtonSet.OK
        );
        return;
      }
    ...
    SpreadsheetApp.getUi().alert('❌ EmailConfigured setting not found!');
  } catch (error) {
...
/**
 * DEBUG: Check all triggers
 */
function debugCheckTriggers() {
  const triggers = ScriptApp.getProjectTriggers();
  ...
  if (triggers.length === 0) {
    ...
    triggers.forEach((trigger, index) => {
      ...
      message += (index + 1) + '. ' + funcName + ' (' + triggerType + ')\n';
      Logger.log((index + 1) + '. Function: ' + funcName + ', Type: ' + triggerType);
    });
  }

  SpreadsheetApp.getUi().alert('🔍 Triggers Debug', message, SpreadsheetApp.getUi().ButtonSet.OK);
}

/**
 * DEBUG: Test consolidated email generation (without sending)
 * This generates the email HTML and logs it to check the structure
 */
function testConsolidatedEmail() {
  try {
    Logger.log('🧪 Testing consolidated email generation...');

    // Get all family members
    const allMembers = getFamilyMembersForEmailReports();

    if (!allMembers || allMembers.length === 0) {
      SpreadsheetApp.getUi().alert('❌ No family members found for email reports');
      return;
    }

    Logger.log('Found ' + allMembers.length + ' family members');

    // Gather consolidated data
    const consolidatedData = getConsolidatedFamilyDashboardData(allMembers);

    if (!consolidatedData.success) {
      SpreadsheetApp.getUi().alert('❌ Failed to generate consolidated data: ' + consolidatedData.error);
      return;
    }

    Logger.log('✅ Consolidated data generated successfully');
    Logger.log('Family Total Invested: ' + consolidatedData.data.familyTotals.invested);
    Logger.log('Family Total Current: ' + consolidatedData.data.familyTotals.current);
    Logger.log('Family Total P&L: ' + consolidatedData.data.familyTotals.pnl);
    Logger.log('Asset Allocation Total: ' + consolidatedData.data.assetAllocation.total);
    Logger.log('Asset Classes: ' + consolidatedData.data.assetAllocation.breakdown.length);

    // Build email HTML
    const htmlBody = buildConsolidatedEmailBody(consolidatedData.data, 'manual');

    Logger.log('✅ Email HTML generated successfully');
    Logger.log('HTML length: ' + htmlBody.length + ' characters');

    // Show success message
    const message = '✅ Consolidated Email Test Successful!\n\n' +
      'Family Members: ' + allMembers.length + '\n' +
      'Total Invested: ₹' + consolidatedData.data.familyTotals.invested.toLocaleString('en-IN') + '\n' +
      'Current Value: ₹' + consolidatedData.data.familyTotals.current.toLocaleString('en-IN') + '\n' +
      'Total P&L: ₹' + consolidatedData.data.familyTotals.pnl.toLocaleString('en-IN') + '\n' +
      'Asset Classes: ' + consolidatedData.data.assetAllocation.breakdown.length + '\n\n' +
      'HTML generated successfully (' + htmlBody.length + ' chars).\n\n' +
      'Ready to send emails!';

    SpreadsheetApp.getUi().alert('✅ Test Passed', message, SpreadsheetApp.getUi().ButtonSet.OK);

    return { success: true, data: consolidatedData.data };
  } catch (error) {
