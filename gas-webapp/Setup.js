...
function oneClickSetup() {
  ...
  // Check if questionnaire sheet exists (if it exists, skip questionnaire)
  const spreadsheet = getSpreadsheet();
  const questionnaireSheet = spreadsheet.getSheetByName('Questionnaire');

  if (!questionnaireSheet) {
    // Questionnaire sheet doesn't exist - show questionnaire dialog (first-time setup)
    log('Questionnaire sheet not found, showing questionnaire dialog');
    ...
    // Questionnaire sheet exists, skip questionnaire and create missing sheets
    log('Questionnaire sheet exists, skipping questionnaire dialog');
...
function createAllSheets() {
  ...
  const sheetsToCreate = [
    // ========== USER-FACING (daily use - appear first in tabs) ==========
    ...
    { name: 'Goals', func: setupGoalsSheet },
    ...
    { name: 'GoalPortfolioMapping', func: setupGoalPortfolioMappingSheet },
...
/**
 * Setup Mutual Fund Data Sheet
 * Creates sheet with headers, then copies data from master DB.
 * No IMPORTRANGE — data is a snapshot, refreshed via refreshMutualFundData().
 */
function setupMutualFundDataSheet() {
  const spreadsheet = getSpreadsheet();
  const sheet = spreadsheet.insertSheet(CONFIG.mutualFundDataSheet);

  // Set headers (same structure as master DB MF_Data)
  // A=Scheme Code, B=Fund Name, C=Category, D=NAV, E=Date, F=Fund House, G=Type, H=Status
  sheet.getRange('A1:H1').setValues([[
    'Scheme Code', 'Fund Name', 'Category', 'NAV', 'Date', 'Fund House', 'Type', 'Status'
  ]]);
  sheet.getRange('A1:H1').setFontWeight('bold').setBackground('#e8eaf6');
  ...
  try {
    ...
    log('Warning: Could not copy MF data from master DB: ' + e.message);
    // Sheet created with headers — data can be populated later via refreshMutualFundData()
  }

  // Hide the sheet (users don't need to see reference data)
  sheet.hideSheet();

  log('MutualFundData sheet created');
  return sheet;
}
...
function formatAllDataRows() {
  ...
  try {
    ...
    log('Skipping UI alert in web app mode');
  }
 ...
 * Creates sheet with headers, then copies data from master DB.
 * No IMPORTRANGE — data is a snapshot, refreshed via refreshStockData().
 */
function setupStockMasterDataSheet() {
  const spreadsheet = getSpreadsheet();
  const sheet = spreadsheet.insertSheet(CONFIG.stockMasterDataSheet);

  // Set headers (same structure as master DB Stock_Data)
  sheet.getRange('A1:J1').setValues([[
    'Symbol', 'Company Name', 'Series', 'ISIN', 'Sector', 'Industry',
    'Market Cap', 'Last Price', 'Date', 'Exchange'
  ]]);
  sheet.getRange('A1:J1').setFontWeight('bold').setBackground('#e8eaf6');
  ...
  try {
    ...
    log('Warning: Could not copy stock data from master DB: ' + e.message);
  }

  // Hide the sheet
  sheet.hideSheet();

  log('StockMasterData sheet created');
  return sheet;
}
...
function setupGoalsSheet() {
  const spreadsheet = getSpreadsheet();
  const sheet = spreadsheet.insertSheet('Goals');

  const headers = [
    'Goal ID',                  // A: GOAL-001
    'Goal Type',                // B: Retirement, Emergency Fund, etc.
    'Goal Name',                // C: User-defined
    'Family Member',            // D: From FamilyMembers
    'Target Amount (₹)',        // E: What you need to save (inflated future value)
    'Target Date',              // F: When you need it
    'Monthly SIP Needed (₹)',   // G: Invest this much monthly
    'Lumpsum Needed (₹)',       // H: Or invest this much one-time
    'Current Allocated (₹)',    // I: Sum from GoalPortfolioMapping
    'Gap Amount (₹)',           // J: Formula (E - I)
    'Progress %',               // K: Formula (I / E * 100)
    'Created Date',             // L: When goal was created
    'Priority',                 // M: High/Medium/Low
    'Status',                   // N: On Track/Behind/Critical
    'Is Active',                // O: Checkbox
    'Notes',                    // P: Text
    'Expected Inflation (%)',   // Q: Hidden - for calculations
  ...
  // Set column widths
  sheet.setColumnWidth(1, 110);  // A: Goal ID
  sheet.setColumnWidth(2, 150);  // B: Goal Type
  sheet.setColumnWidth(3, 200);  // C: Goal Name
  sheet.setColumnWidth(4, 130);  // D: Family Member
  sheet.setColumnWidth(5, 150);  // E: Target Amount
  sheet.setColumnWidth(6, 110);  // F: Target Date
  sheet.setColumnWidth(7, 160);  // G: Monthly SIP Needed
  sheet.setColumnWidth(8, 150);  // H: Lumpsum Needed
  sheet.setColumnWidth(9, 150);  // I: Current Allocated
  sheet.setColumnWidth(10, 130); // J: Gap Amount
  sheet.setColumnWidth(11, 100); // K: Progress %
  sheet.setColumnWidth(12, 130); // L: Created Date
  sheet.setColumnWidth(13, 90);  // M: Priority
  sheet.setColumnWidth(14, 110); // N: Status
  sheet.setColumnWidth(15, 80);  // O: Is Active
  sheet.setColumnWidth(16, 200); // P: Notes
  sheet.setColumnWidth(17, 50);  // Q: Expected Inflation (hidden)
  sheet.setColumnWidth(18, 50);  // R: Expected CAGR (hidden)
  ...
  // Format currency columns
  sheet.getRange('E:E').setNumberFormat('#,##0.00');  // E: Target Amount
  sheet.getRange('G:G').setNumberFormat('#,##0.00');  // G: Monthly SIP Needed
  sheet.getRange('H:H').setNumberFormat('#,##0.00');  // H: Lumpsum Needed
  sheet.getRange('I:I').setNumberFormat('#,##0.00');  // I: Current Allocated
  sheet.getRange('J:J').setNumberFormat('#,##0.00');  // J: Gap Amount
  sheet.getRange('K:K').setNumberFormat('0.00%');     // K: Progress %
  sheet.getRange('Q:Q').setNumberFormat('0.00%');     // Q: Expected Inflation (hidden)
  ...
  // Set tab color (Orange for goals)
  sheet.setTabColor('#f59e0b');

  log('Goals sheet created successfully');
  return sheet;
}
...
function setupGoalPortfolioMappingSheet() {
  const spreadsheet = getSpreadsheet();
  const sheet = spreadsheet.insertSheet('GoalPortfolioMapping');
  ...
  // Set column widths
  sheet.setColumnWidth(1, 120);  // Mapping ID
  sheet.setColumnWidth(2, 110);  // Goal ID
  sheet.setColumnWidth(3, 220);  // Goal Name
  ...
  // Format allocation column
  sheet.getRange('F:F').setNumberFormat('0.00%');  // Allocation %

  // Apply standard formatting
  applyStandardFormatting(sheet);

  // Set tab color (Amber for mappings)
