...
function getPortfolioFunds(portfolioId) {
  try {
    ...
    // G=Current Allocation %, H=Current Value ₹, I=Target Allocation %, J=Target Value ₹,
    // K=Ongoing SIP ₹, L=Rebalance SIP ₹, M=Target Lumpsum ₹, N=Rebalance Lumpsum ₹, O=Buy/Sell ₹, P=P&L ₹
    const data = portfolioSheet.getRange(4, 1, lastRow - 3, 16).getValues(); // Get all 16 columns
    const funds = [];
    ...
    data.forEach((row, index) => {
      if (row[0]) {  // If Fund Code exists (Column A)
        funds.push({
          ...
          units: row[2] || 0,        // Column C: Units
          avgPrice: row[3] || 0,     // Column D: Avg NAV ₹
          currentNav: row[4] || 0,   // Column E: Current NAV ₹
          invested: row[5] || 0,     // Column F: Investment ₹
          currentValue: row[7] || 0  // Column H: Current Value ₹
        });
      }
    });
