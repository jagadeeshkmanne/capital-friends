...
function updateExistingPortfolioPLFormulas() {
  try {
    ...
    const response = ui.alert(
      'Update Portfolio Formulas',
      'This will update formulas for all existing portfolios:\n\n' +
      '• Smart Total Investment calculation (uses Original Investment if set, else auto-calculates)\n' +
      '• Fixed P&L formulas (Total, Realized, Unrealized)\n' +
      '• Proper SWITCH transaction handling\n\n' +
    ...
    const lastRow = metadataSheet.getLastRow();
    if (lastRow <= 3) {
      showInfo('No portfolios found to update');
      return;
    }
    ...
    showSuccess(`Updated formulas for ${updatedCount} portfolio(s).\n\n` +
                `✓ Smart Total Investment calculation\n` +
                `✓ Fixed P&L formulas\n` +
                `✓ Proper transaction handling\n\n` +
                `Total P&L now equals: Current Value - Total Investment`);
...
function fixAllPortfolioSheetFormulas() {
  try {
    const ui = SpreadsheetApp.getUi();
    const response = ui.alert(
      'Fix Portfolio Sheet Formulas',
      'This will update formulas in ALL portfolio sheets:\n\n' +
      '✓ Fix Fund Name lookup (use MutualFundData instead of TransactionHistory)\n' +
      '✓ Fix percentage formatting (10000% → 100%)\n' +
      '✓ Fix incomplete SUM ranges\n' +
      '✓ Fix SIP/Lumpsum VLOOKUP errors\n\n' +
    ...
    message += 'Fixed:\n';
    message += '• Fund Name formulas (now use MutualFundData)\n';
    message += '• Percentage formatting (now shows correctly)\n';
    message += '• SUM ranges (now complete)\n';
    message += '• SIP/Lumpsum VLOOKUP formulas\n';
    message += '• Rebalance threshold gate (L, N, O columns)\n';
    message += '• ATH NAV and % Below ATH columns (R, S)\n';
