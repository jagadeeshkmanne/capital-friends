...
test('breakdown rows add up to the totals', () => {
  const today = new Date(2026, 8, 10)
  const m = buildFundsModel({ portfolios, holdings: mfHoldings, transactions: mfTransactions, today })
  const sumP = m.breakdown.byPortfolio.reduce((s, r) => s + r.current, 0)
  const sumM = m.breakdown.byMember.reduce((s, r) => s + r.current, 0)
  assert.ok(Math.abs(sumP - m.totals.currentValue) < 1)
  assert.ok(Math.abs(sumM - m.totals.currentValue) < 1)
